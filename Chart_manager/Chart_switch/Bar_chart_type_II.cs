...
namespace evidence_clip_about_public_transport.Chart_manager.Chart_switch
{
    ...
    /// Bar Chart type II
    /// </summary>
    public class Bar_chart_type_II:I_Chart_switch
    {
        /// <summary>
        ...
        public void chart_with_data(FormsPlot chart, DataGridView data, string header)
        {
            ...
            {
                position_++;
                ticks[position_ - 1] = new Tick(position_, row.Cells[1].Value.ToString()+" ("+ row.Cells[0].Value.ToString()+")");
                chart.Plot.Add.Bar(position: position_, value: Double.Parse(row.Cells[2].Value?.ToString()));
            }
