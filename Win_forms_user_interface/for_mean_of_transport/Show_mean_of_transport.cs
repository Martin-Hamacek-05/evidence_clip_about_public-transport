...
{
    public partial class Show_mean_of_transport : Form,I_List_from_table
    {
        
        private readonly string[] headers_in_czech = { "název dopravního prostředku", "zkrácený název" };

        public Show_mean_of_transport()
        {
            InitializeComponent();
        ...
        {
            Edit_mean_of_transport new_stop = new Edit_mean_of_transport();
            new_stop.Show();
        }
        private void load_button_Click(object sender, EventArgs e)
        {
            I_DAO_Mean_of_transport stops = Database_server_switch.dAO_Mean_Of_Transport();
            data_from_select.DataSource = stops.get_all_mean_of_transport().Tables[0];
        ...
        {
            I_DAO_Mean_of_transport load = Database_server_switch.dAO_Mean_Of_Transport();
            Edit_mean_of_transport edit = new Edit_mean_of_transport();
            ...
            if (selected != -1)
            {
                ...
                edit.set_mean_of_transport(load.get_mean_of_transport(Convert.ToInt32(selected_row.Cells[0].Value)));
                edit.Show();
            }
        }
