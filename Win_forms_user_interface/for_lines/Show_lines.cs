...
namespace evidence_clip_about_public_transport.Win_forms_user_interface.for_lines
{
    public partial class Show_lines : Form,I_List_from_table
    {
        ...
        private void search_button_Click(object sender, EventArgs e) 
        {
            try
            {
                data_from_select.Columns[2].HeaderText = "name_line";
                BindingSource bs = new BindingSource();
                bs.DataSource = data_from_select.DataSource;
                bs.Filter = data_from_select.Columns[2].HeaderText.ToString() + " LIKE '%" + search_textbox.Text + "%'";
                data_from_select.Columns[2].HeaderText = headers_in_czech[1];
